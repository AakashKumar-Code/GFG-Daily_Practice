...
class Solution{
  public:
    ...
    // Add the current node's value to the sum at its horizontal distance
    sums[hd] += root->data;
...
vector<int> verticalSum(Node *root) {
    ...
    // Map to store the vertical sum at each horizontal distance
    map<int, int> sums;

    // Calculate vertical sums
    verticalSumUtil(root, 0, sums);

    // Extract the sums into the result vector
